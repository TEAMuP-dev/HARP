...
Font ToolbarSliderStyle::getTextButtonFont(TextButton &button,
                                           int buttonHeight) {
  return Font(2.0f, Font::plain);
}

void ToolbarSliderStyle::drawRotarySlider(juce::Graphics &g, int x, int y,
...
Font ButtonLookAndFeel::getTextButtonFont(TextButton &button,
                                          int buttonHeight) {
  return Font(2.0f, Font::plain);
}

void ButtonLookAndFeel::drawButtonBackground(Graphics &g, Button &button,
...
void ButtonLookAndFeel::drawButtonText(Graphics &g, TextButton &button,
  ...
  const int yIndent =
      jmin(4, button.proportionOfHeight(0.1f)); // Reduced indent proportion
  ...
  if (textWidth > 0) {
    g.setFont(Font(16.0, Font::plain));
    g.drawFittedText(button.getButtonText(), leftIndent,
                     yIndent, // top-left position with new indent
                     button.getWidth() - 2 * leftIndent,
                     button.getHeight() - 2 * yIndent, // new width and height
...
Font ComboBoxLookAndFeel::getTextButtonFont(TextButton &button,
                                            int buttonHeight) {
  return Font(2.0f, Font::plain);
}

void ComboBoxLookAndFeel::drawComboBox(Graphics &g, int width, int height,
  ...
  auto arrowH =
      static_cast<float>(height * 0.35f * 0.5f); // Scale down the height by 50%

  Path path;
  path.startNewSubPath(arrowX,
                       height * 0.3f + arrowH * 0.5f); // Center the arrow
  path.lineTo(arrowX + arrowH * 0.6f,
              height * 0.7f - arrowH * 0.5f); // Adjust the arrow point
  path.lineTo(arrowX - arrowH * 0.6f,
              height * 0.7f - arrowH * 0.5f); // Adjust the arrow point
  path.closeSubPath();
  path.applyTransform(AffineTransform::rotation(
      -M_PI, arrowX, height * 0.5f)); // Rotate the arrow to point downwards
...
void ComboBoxLookAndFeel::drawPopupMenuItem(
