...
public:
  ...
  void
  didAddMusicalContextToDocument(ARADocument *,
                                 ARAMusicalContext *musicalContext) override;
  ...
  void didAddRegionSequenceToDocument(ARADocument *,
                                      ARARegionSequence *) override;
  void willRemoveRegionSequenceFromDocument(
      ARADocument *, ARARegionSequence *regionSequence) override;
  ...
  void onHideRegionSequences(
      const std::vector<ARARegionSequence *> &regionSequences) override;
...
private:
  struct RegionSequenceViewKey {
    explicit RegionSequenceViewKey(ARARegionSequence *regionSequence)
        : orderIndex(regionSequence->getOrderIndex()),
          sequence(regionSequence) {}

    bool operator<(const RegionSequenceViewKey &other) const {
      return std::tie(orderIndex, sequence) <
             std::tie(other.orderIndex, other.sequence);
    }
  ...
  template <typename T>
  void layOutVertically(Rectangle<int> bounds, T &components,
  ...
  std::map<RegionSequenceViewKey, unique_ptr<RegionSequenceView>>
      regionSequenceViews;
