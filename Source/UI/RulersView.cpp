...
void RulersView::CycleMarkerComponent::paint(Graphics &g) {
  ...
  g.drawRoundedRectangle(bounds.getX(), bounds.getY(), bounds.getWidth(),
                         bounds.getHeight(), 6.0f, 2.0f);
}

RulersView::RulersView(PlayHeadState &playHeadStateIn,
  ...
  setTooltip("Double-click to start playback, click to stop playback or to "
             "reposition, drag horizontal range to set cycle.");
...
void RulersView::paint(Graphics &g) {
  ...
  g.drawRect(drawBounds.getX(), rulerHeight, drawBounds.getRight(),
             rulerHeight);
  ...
  if (selectedMusicalContext != nullptr) {
    ...
    if (tempoReader && chordsReader) {
      ...
      for (auto itChord = chordsReader.begin(); itChord != chordsReader.end();
           ++itChord) {
        if (interpreter.isNoChord(*itChord))
          continue;

        const auto chordStartTime =
            (itChord == chordsReader.begin())
                ? 0
                : tempoConverter.getTimeForQuarter(itChord->position);
        ...
        if (std::next(itChord) != chordsReader.end()) {
          const auto nextChordStartTime =
              tempoConverter.getTimeForQuarter(std::next(itChord)->position);
        ...
        g.drawText(
            convertARAString(interpreter.getNameForChord(*itChord).c_str()),
            chordRect.withTrimmedLeft(2), Justification::centredLeft);
      }
    }

    // beat ruler: evaluates tempo and bar signatures to draw a line for each
    // beat
    ...
    if (barSignaturesReader) {
      ...
      const double beatStart = barSignaturesConverter.getBeatForQuarter(
          tempoConverter.getQuarterForTime(drawStartTime));
      const double beatEnd = barSignaturesConverter.getBeatForQuarter(
          tempoConverter.getQuarterForTime(drawEndTime));
      const int endBeat = roundToInt(std::floor(beatEnd));
      RectangleList<int> rects;

      for (int beat = roundToInt(std::ceil(beatStart)); beat <= endBeat;
           ++beat) {
        const auto quarterPos = barSignaturesConverter.getQuarterForBeat(beat);
        const int x = timeToViewScaling.getXForTime(
            tempoConverter.getTimeForQuarter(quarterPos));
        const auto barSignature =
            barSignaturesConverter.getBarSignatureForQuarter(quarterPos);
        const int lineWidth = (quarterPos == barSignature.position)
                                  ? heavyLineWidth
                                  : lightLineWidth;
        const int beatsSinceBarStart = roundToInt(
            barSignaturesConverter.getBeatDistanceFromBarStartForQuarter(
                quarterPos));
        const int lineHeight =
            (beatsSinceBarStart == 0) ? rulerHeight : rulerHeight / 2;
        rects.addWithoutMerging(Rectangle<int>(x - lineWidth / 2,
                                               2 * rulerHeight - lineHeight,
  ...
  // time ruler: one tick for each second
  {
    ...
    for (auto time = std::floor(drawStartTime); time <= drawEndTime;
         time += 1.0) {
      const int lineWidth =
          (std::fmod(time, 60.0) <= 0.001) ? heavyLineWidth : lightLineWidth;
      const int lineHeight =
          (std::fmod(time, 10.0) <= 0.001) ? rulerHeight : rulerHeight / 2;
      rects.addWithoutMerging(Rectangle<int>(
          timeToViewScaling.getXForTime(time) - lineWidth / 2,
          bounds.getHeight() - lineHeight, lineWidth, lineHeight));
    }
...
void RulersView::mouseUp(const MouseEvent &m) {
  auto playbackController =
      araDocument.getDocumentController()->getHostPlaybackController();

  if (playbackController != nullptr) {
    const auto startTime =
        timeToViewScaling.getTimeForX(jmin(m.getMouseDownX(), m.x));
    const auto endTime =
        timeToViewScaling.getTimeForX(jmax(m.getMouseDownX(), m.x));
...
void RulersView::selectMusicalContext(
    ARAMusicalContext *newSelectedMusicalContext) {
  if (auto *oldSelection =
...
void RulersView::zoomLevelChanged(double) { repaint(); }

void RulersView::doUpdateMusicalContextContent(ARAMusicalContext *,
                                               ARAContentUpdateScopes) {
  repaint();
}
...
void RulersView::updateCyclePosition() {
  if (selectedMusicalContext != nullptr) {
    ...
    const auto loopStartTime = tempoConverter.getTimeForQuarter(
        playHeadState.loopPpqStart.load(std::memory_order_relaxed));
    const auto loopEndTime = tempoConverter.getTimeForQuarter(
        playHeadState.loopPpqEnd.load(std::memory_order_relaxed));
