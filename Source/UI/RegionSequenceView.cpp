...
RegionSequenceView::RegionSequenceView(EditorView &editorView,
                                       ...
                                       WaveformCache &cache)
    : araEditorView(editorView), timeToViewScaling(scaling), regionSequence(rs),
      waveformCache(cache) {
...
void RegionSequenceView::willUpdateRegionSequenceProperties(
    ARARegionSequence *, ARARegionSequence::PropertiesPtr newProperties) {
...
void RegionSequenceView::zoomLevelChanged(double) { resized(); }
  ...
  for (auto &pbr : playbackRegionViews) {
    ...
    pbr.second->setBounds(
        getLocalBounds()
            ...
            .withWidth(timeToViewScaling.getXForTime(
...
void RegionSequenceView::updatePlaybackDuration() {
  const auto iter =
      std::max_element(playbackRegionViews.begin(), playbackRegionViews.end(),
                       [](const auto &a, const auto &b) {
                         return a.first->getEndInPlaybackTime() <
