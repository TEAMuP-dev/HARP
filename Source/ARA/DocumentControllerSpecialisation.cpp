/**
  ...
  * This controller is needed for any ARA plugin and handles functions for editing and playback of audio, 
  * analysis of content from the host, and it maintains the ARA model graph. 
  * More information can be found on the offical JUCE documentation: https://docs.juce.com/master/classARADocumentControllerSpecialisation.html#details
  * @author JUCE, aldo aguilar, hugo flores garcia
  */
 
#include "DocumentControllerSpecialisation.h"

void TensorJuceDocumentControllerSpecialisation::willBeginEditing (ARADocument*)
{
...
ARAAudioModification* TensorJuceDocumentControllerSpecialisation::doCreateAudioModification (ARAAudioSource* audioSource,
                                                                                                 ARA::ARAAudioModificationHostRef hostRef,
                                                                                                 const ARAAudioModification* optionalModificationToClone) noexcept
{
    return new AudioModification (audioSource,
                                  hostRef,
                                  static_cast<const AudioModification*> (optionalModificationToClone));
}

ARAPlaybackRenderer* TensorJuceDocumentControllerSpecialisation::doCreatePlaybackRenderer() noexcept
{
    return new PlaybackRenderer (getDocumentController(), *this);
}

EditorRenderer* TensorJuceDocumentControllerSpecialisation::doCreateEditorRenderer() noexcept
{
    return new EditorRenderer (getDocumentController(), &previewState, *this);
}

bool TensorJuceDocumentControllerSpecialisation::doRestoreObjectsFromStream (ARAInputStream& input,
                                                                                const ARARestoreObjectsFilter* filter) noexcept
{
    ...
    for (int64 i = 0; i < numAudioModifications; ++i)
    {
        ...
        auto audioModification = filter->getAudioModificationToRestoreStateWithID<AudioModification> (persistentID.getCharPointer());
...
bool TensorJuceDocumentControllerSpecialisation::doStoreObjectsToStream (ARAOutputStream& output,
                                                                            const ARAStoreObjectsFilter* filter) noexcept
{
    // This example implementation only deals with audio modification states
    const auto& audioModificationsToPersist { filter->getAudioModificationsToStore<AudioModification>() };
...
ScopedTryReadLock TensorJuceDocumentControllerSpecialisation::getProcessingLock()
{
