/**
  ...
  * 
  * Audio modification class started from the ARAPluginDemo.h file provided by juce. This class now also manages offline processing.
 ...
 * @param optionalModificationToClone A pointer to an optional modification to clone
 */
AudioModification::AudioModification(ARAAudioSource* audioSource,
...
bool AudioModification::isDimmed() const {
    return dimmed;
}

void AudioModification::setDimmed(bool shouldDim) {
    dimmed = shouldDim;
}

std::string AudioModification::getSourceName() {
    return mAudioSourceName;
}
...
bool AudioModification::getIsModified() {
    return mIsModified;
}
