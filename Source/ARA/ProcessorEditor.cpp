/**
  ...
  * 
  * @brief The Processor Editor is the main interaction from users into the plugin.
  * We utilize the Processor Editor to manage the UI of the plugin and control all user input.
  * The Processor Editor manages the UI components for user input, and manages a callback
  * to send UI information to the deeplearning model. 
  * @author JUCE, aldo aguilar, hugo flores garcia
  */
 
 #include "ProcessorEditor.h"

TensorJuceProcessorEditor::TensorJuceProcessorEditor (ARADemoPluginAudioProcessorImpl& p, EditorRenderer* er)
...
void TensorJuceProcessorEditor::comboBoxChanged (ComboBox* box)
{
    if (box == &modeBox)
    {
        DBG ("TensorJuceProcessorEditor::comboBoxChanged mode changed: " << box->getSelectedId());
    }
}
...
void TensorJuceProcessorEditor::sliderValueChanged(Slider* slider)
{
    if (slider == &temp1Dial)
    {
        DBG("TensorJuceProcessorEditor::sliderValueChanged temp1 dial value:  " << slider->getValue());
    }

    else if (slider == &temp2Dial)
    {
        DBG("TensorJuceProcessorEditor::sliderValueChanged temp2 dial value:  " << slider->getValue());
    }
}
