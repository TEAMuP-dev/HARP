...
void ARADemoPluginAudioProcessorImpl::prepareToPlay (double sampleRate, int samplesPerBlock)
{
...
bool ARADemoPluginAudioProcessorImpl::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void ARADemoPluginAudioProcessorImpl::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
...
juce::AudioProcessor::BusesProperties ARADemoPluginAudioProcessorImpl::getBusesProperties()
{
    return BusesProperties().withInput  ("Input",  AudioChannelSet::stereo(), true).withOutput("Output", AudioChannelSet::stereo(), true);
}
