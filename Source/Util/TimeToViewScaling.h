 ...
 *
 * @brief Class to convert between time in a playback region and pixels on
 * screen.
...
public:
  ...
  void setZoomLevel(double pixelPerSecond) {
    ...
    listeners.call(
        [this](Listener &l) { l.zoomLevelChanged(zoomLevelPixelPerSecond); });
  }
  ...
  double getTimeForX(int x) const { return x / zoomLevelPixelPerSecond; }
