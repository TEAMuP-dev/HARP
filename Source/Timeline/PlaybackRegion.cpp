...
PlaybackRegionView::PlaybackRegionView(ARAEditorView& editorView, ARAPlaybackRegion& region, WaveformCache& cache)
: araEditorView (editorView), playbackRegion (region), waveformCache (cache), previewRegionOverlay (*this) {
    ...
    auto audioModificationInstance = dynamic_cast<AudioModification*>(audioModification);
    
    
    audioSource = playbackRegion.getAudioModification()->getAudioSource();

    waveformCache.getOrCreateThumbnail (audioSource, audioModificationInstance).addChangeListener (this);
...
PlaybackRegionView::~PlaybackRegionView()
{
    ...
    waveformCache.getOrCreateThumbnail (audioSource, dynamic_cast<AudioModification*>(playbackRegion.getAudioModification())).removeChangeListener (this);
}
...
void PlaybackRegionView::mouseDown(const juce::MouseEvent& m)
{
    ...
    auto& previewState = ARADocumentControllerSpecialisation::getSpecialisedDocumentController<TensorJuceDocumentControllerSpecialisation> (playbackRegion.getDocumentController())->previewState;
...
void PlaybackRegionView::didEnableAudioSourceSamplesAccess(ARAAudioSource*, bool)
{
    repaint();
}

void PlaybackRegionView::willUpdatePlaybackRegionProperties(ARAPlaybackRegion*, ARAPlaybackRegion::PropertiesPtr newProperties)
{
...
void PlaybackRegionView::didUpdatePlaybackRegionContent(ARAPlaybackRegion*, ARAContentUpdateScopes)
{
    repaint();
}
...
void PlaybackRegionView::paint(juce::Graphics& g)
{
    ...
    {
        auto& thumbnail = waveformCache.getOrCreateThumbnail (playbackRegion.getAudioModification()->getAudioSource(), dynamic_cast<AudioModification*>(playbackRegion.getAudioModification()));
...
TensorJuceDocumentControllerSpecialisation* PlaybackRegionView::getDocumentController() const
{
    return ARADocumentControllerSpecialisation::getSpecialisedDocumentController<TensorJuceDocumentControllerSpecialisation> (playbackRegion.getDocumentController());
}
