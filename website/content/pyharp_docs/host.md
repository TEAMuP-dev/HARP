...
## Gradio Endpoints
Gradio endpoints are the most convenient solution for a PyHARP application. It will give you access to GPU computation with ZeroGPU hardware if you are a PRO subscriber, at no additional charge.
...
6. Here are the suggested configurations in the related files.
   ...
   - `requirements.txt`
     ...
     This is the place where you put all the **pip** packages. You should also put in the latest PyHARP:
     ```
   ...
   - `packages.txt`
     ...
     This is there to put **apt-get install** Debian package list, which are required by some models.
...
Using Docker endpoints can help you fix this issue. Docker will allow you to customize deployments, making room for potential patches and fixes. However, the ZeroGPU hardware will not be available in Docker spaces, and you will need to pay for GPU usage.
...
5. Configurations
    ...
    - `requirements.txt` and `packages.txt`
      ...
      Similar in the Gradio endpoint setting, they are for **pip** and **apt-get** packages. You are going to install them through the `Dockerfile` introduced in the next step.
    ...
    - `Dockerfile`
      ...
      Here is an example `Dockerfile` configuration that installs general packages and fixes the `madmom` package.
...
---
Your PyHARP app will then begin running at `https://huggingface.co/spaces/<USERNAME>/<SPACE_NAME>`. The shorthand `<USERNAME>/<SPACE_NAME>` can also be used within HARP to reference the endpoint. The app deployed using two methods will have the same UI and functionality.
...
For more information, please refer to the offical Hugging Face document on [Spaces](https://huggingface.co/docs/hub/spaces).
