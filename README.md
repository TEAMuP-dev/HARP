...
All of the good parts of HARP, none of the bad ones!
...
HARP is a sample editor that allows for **h**osted, **a**synchronous, **r**emote **p**rocessing of audio with deep learning models. HARP works by routing audio through [Gradio](https://www.gradio.app) endpoints. Because Gradio apps can be hosted locally or in the cloud (e.g., HuggingFace Spaces), HARP lets users of Digital Audio Workstations capable of connecting with external sample editors (e.g. [Reaper](https://www.reaper.fm), Ableton Live and Logic Pro X) access large state-of-the-art models in the cloud, without breaking their within-DAW workflow.

# OS and DAW compatibility
HARP has been tested on arm-based Mac computers running Mac OS (versions 13.0 and 13.4), using [REAPER](https://www.reaper.fm) and Logic Pro X.
...
* Drag HARP.app to the `Applications/` folder to install. 

## Windows & Linux
The windows build is still under development. There are no current plans to support Linux

# TODO: Setting up HARP as an External Sample Editor in Reaper 
# TODO: Setting up HARP as an External Sample Editor in Logic Pro X
# TODO: Setting up HARP as an External Sample Editor in Ableton Live
