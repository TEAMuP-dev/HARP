    ...
    OpResult prepareProcessingPayload(juce::String& payloadJson)
    {
        ...
        for (const auto& currentUuid : uuidsInOrder)
        {
            ...
            if (!element)
            {
                ...
                error.devMessage = "Control with ID: " + currentUuid.toString() + " not found in controlsInfo.";
                return OpResult::fail(error);
            }
            ...
            else
            {
                ...
                error.devMessage = "Unsupported control type for control with UUID: " + currentUuid.toString();
                return OpResult::fail(error);
            }
